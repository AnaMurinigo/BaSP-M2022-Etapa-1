window.onload=function(){
  ...
  submit.onclick = function (e) {
    e.preventDefault();
    //I want to do the onblur of every field, cause in other case I have to click every single one if they are already onload to submit:
     firstNameBlur(); lastNameBlur();  dniBlur(); birthdateBlur();  telBlur(); 
     addressBlur(); cityBlur(); zipBlur();  emailBlur(); pwdBlur(); pwd2Blur();
    ...
    + "&dni="+ dni.value + "&dob="+ dateFormatMDY(birthdate.value) +"&phone="+ tel.value + "&address="+ address.value+ "&city="+ city.value + "&zip=" + zip.value + "&password="+pwd.value;
      console.log(url);
      apiRequest(url);
    } else {
